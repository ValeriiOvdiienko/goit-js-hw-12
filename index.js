...
</li>`).join("")}function p(e,r){e.insertAdjacentHTML("beforeend",B(r)),O.refresh()}function C(e){e.innerHTML=""}function L(e){e.classList.remove("hidden")}function b(e){e.classList.add("hidden")}function P(e){e.classList.remove("hidden")}function w(e){e.classList.add("hidden")}function l(e){g.error({position:"topRight",timeout:5e3,message:e,icon:!1,messageColor:"#ffffff",backgroundColor:"#992a2a"})}function v(e){g.info({position:"topRight",timeout:5e3,message:e,icon:!1,messageColor:"#ffffff",backgroundColor:"#4e75ff"})}function H(){const e=document.querySelector(".gallery-item");if(e){const{height:r}=e.getBoundingClientRect(),o=r*2;window.scrollBy({top:o,behavior:"smooth"})}}const D=document.querySelector(".form"),h=document.querySelector(".gallery"),f=document.querySelector(".loader"),c=document.querySelector(".load-more-btn");let u="",n=0;const m=15;let d=0,M=0;const S="We're sorry, but you've reached the end of search results.";D.addEventListener("submit",G);async function G(e){if(e.preventDefault(),u=e.target.elements[0].value,u.trim()===""){l("Yoyr search query is empty");return}w(c),C(h),L(f),n=1;try{const o=await y(u,n);if(console.log(o),d=o.totalHits,M=Math.ceil(d/m),o.totalHits.length===0){l("Sorry, there are no images matching your search query. Please try again!");return}p(h,o.hits),e.target.reset(),d>m?P(c):v(S)}catch(o){l(o.message)}finally{b(f)}}c.addEventListener("click",T);async function T(e){w(c),n++,n<=M?P(c):v(S);try{L(f);const r=await y(u,n);p(h,r.hits),H()}catch(r){l(r.message)}finally{b(f)}}
